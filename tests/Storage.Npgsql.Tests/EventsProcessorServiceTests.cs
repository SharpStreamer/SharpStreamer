...
public class EventsProcessorServiceTests
{
    ...
    public async Task ProcessEvents_WhenExceptionMessageTooLongAndNextRetryTimeReturned_TruncatesErrorMessageAndSetNextRetryCorrectly()
    {
        ...
        DateTimeOffset expectedNextExecutionTimestamp = new DateTimeOffset(2026, 12, 12, 5, 2, 3, TimeSpan.Zero);
        ...
        _eventProcessor
            .ProcessEvent(Arg.Is(events[0]), Arg.Any<Dictionary<Guid, EventStatus>>())
            .Returns((events[0].Id, EventStatus.Failed, longErrorMessage, 123));
