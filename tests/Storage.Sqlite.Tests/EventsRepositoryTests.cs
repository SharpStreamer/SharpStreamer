...
namespace Storage.Sqlite.Tests;

[Collection(nameof(GlobalCollection))]
public class EventsRepositoryTests : DatabaseTest
{
    private readonly Fixture _fixture;
    private readonly EventsRepository<SqliteTestingDbContext> _eventsRepository;
    ...
    public EventsRepositoryTests(
        SqliteDbFixture sqliteDbFixture,
        DataFixtureConfig dataFixtureConfig) : base(sqliteDbFixture)
    {
        _dbContextFactory = sqliteDbFixture.DbContextFactory;
        _fixture = dataFixtureConfig.Fixture;
        _migrationService = Substitute.For<IMigrationService>();
        ILogger<EventsRepository<SqliteTestingDbContext>> logger =
            Substitute.For<ILogger<EventsRepository<SqliteTestingDbContext>>>();
        _sharpStreamerOptions = Substitute.For<IOptions<SharpStreamerOptions>>();
        _timeService = Substitute.For<ITimeService>();
        _eventsRepository = new EventsRepository<SqliteTestingDbContext>(
            (SqliteTestingDbContext)DbContext,
            logger,
    ...
    public async Task MarkPostProcessing_WhenSingleEventCalled_MarksPassedEvents(string? errorMessage)
    {
        ...
        DbContext.Set<ReceivedEvent>().AddRange(receivedEvent);
        await DbContext.SaveChangesAsync();
    ...
    public async Task GetPredecessorIds_ReturnsCorrectIds()
    {
        ...
        DbContext.Set<ReceivedEvent>().AddRange(allEvents);
        await DbContext.SaveChangesAsync();
    ...
    public async Task GetEventsToPublish_ReturnsAndMarksCorrectEvents()
    {
        ...
        DbContext.Set<PublishedEvent>().AddRange(allEvents);
        await DbContext.SaveChangesAsync();
    ...
    public async Task MarkPostPublishAttempt_MarksEventsAsSucceeded()
    {
        ...
        DbContext.Set<PublishedEvent>().AddRange(publishedEvents);
        await DbContext.SaveChangesAsync();
    ...
    public async Task SaveConsumedEvents_InsertsNewEventsAndIgnoresConflict()
    {
        ...
        DbContext.Set<ReceivedEvent>().Add(existingEvent);
        await DbContext.SaveChangesAsync();
    ...
    public async Task DeleteProducedEventsById_DeletesCorrectEvents()
    {
        ...
        (await DbContext.Set<PublishedEvent>().CountAsync()).Should().Be(3);
    ...
    public async Task DeleteReceivedEventsById_DeletesCorrectEvents()
    {
        ...
        (await DbContext.Set<ReceivedEvent>().CountAsync()).Should().Be(3);
